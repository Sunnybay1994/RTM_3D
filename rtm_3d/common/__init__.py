...
def parser_ini(mode=0):
    ...
    group2.add_argument('--steps',type=str,help="Manually Select which steps are involved. 'g' for generate data; 'f' for source(forward) wavefield; 'p' for prepareing backward data; 'b' for receiver(backward) wavefield; 'i' for cross-corrlation image condition; 'z' for cal zero-offset backward wvf(image condition); 'c' for clean middle result. if result exists, donot conduct 'gfpbi', use uppercase for force run. Default: Use 'gfpbic' for a whole multi-offset workflow, 'gfzc' for a whole zero-offset workflow, 'gfpbizc' for both.")
