...
def parser_ini(mode=0):
    ...
    group1.add_argument('--suffix',default='',dest='suffix',help="Add a suffix to taskname, to specify tasks with same model and parameter.")
    ...
    group2.add_argument('--steps',type=str,help="Manually Select which steps are involved. 'g' for generate data; 'f' for source(forward) wavefield; 'b' for receiver(backward) wavefield; 'i' for cross-corrlation image condition; 'z' for zero-offset data image condition; 'c' for clean middle results. Add 'p' right before 'b' or 'z' to prepare backward data. If result exists, donot conduct 'gfbiz', use uppercase to force running. Default: Use 'gfpbic' for a whole multi-offset workflow (defult for '-m m'), 'gfpzc' for a whole zero-offset workflow (defult for '-m z'), 'gfpbipzc' for both (defult for '-m mz').")
