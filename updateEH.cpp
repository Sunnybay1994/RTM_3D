    ...
    for (i=main_start; i<main_end; ++i)
    {
        for (j=nyPML;j<ny-nyPML;++j)
        {
            for (k=nzPML;k<nz-nzPML;++k)
            {
                ...
                sig_halfyz = (sig(i,j,k) + sig(i,j+1,k+1) + sig(i,j+1,k) + sig(i,j,k+1))/4;
                eps_halfyz = (eps(i,j,k) + eps(i,j+1,k+1) + eps(i,j+1,k) + eps(i,j,k+1))/4;
                ...
                sig_halfxz = (sig(i,j,k) + sig(i+1,j,k+1) + sig(i+1,j,k) + sig(i,j,k+1))/4;
                eps_halfxz = (eps(i,j,k) + eps(i+1,j,k+1) + eps(i+1,j,k) + eps(i,j,k+1))/4;

                // if(i>=9 && i<=15 && j == 50 && k>=28 && k<=32){
                //     FILE *ftest;
                //     ftest = fopen("test_1.txt","at");
                //     fprintf(ftest, "MAIN: %d %d %d %e %e\n", i, j, k, eps(i,j,k), eps_halfyz);
                //     fclose(ftest);
                ...
                sig_halfxy = (sig(i,j,k) + sig(i+1,j+1,k) + sig(i+1,j,k) + sig(i,j+1,k))/4;
                eps_halfxy = (eps(i,j,k) + eps(i+1,j+1,k) + eps(i+1,j,k) + eps(i,j+1,k))/4;
...
void PML_parameter_H(int nn, int i, int j, int k) //nn  for X(0) Y(2) Z(3)
{
    int i0 = i - rangex[myRank] + order;
    if(nn == 0)
    {
        // sigxmax = (m+1) / (150 * pi * sqrt((eps(i0,j,k)+eps(i0+1,j,k))/2/eps0) * dx);
        // sigymax = (m+1) / (150 * pi * sqrt((eps(i0,j,k)+eps(i0+1,j,k))/2/eps0) * dy);
        // sigzmax = (m+1) / (150 * pi * sqrt((eps(i0,j,k)+eps(i0+1,j,k))/2/eps0) * dz);
        sigxmax = (m+1) / (150 * pi * sqrt(1) * dx);
        sigymax = (m+1) / (150 * pi * sqrt(1) * dy);
        sigzmax = (m+1) / (150 * pi * sqrt(1) * dz);
    }
    if(nn == 1)
    {
        // sigxmax = (m+1) / (150 * pi * sqrt((eps(i0,j,k)+eps(i0,j+1,k))/2/eps0) * dx);
        // sigymax = (m+1) / (150 * pi * sqrt((eps(i0,j,k)+eps(i0,j+1,k))/2/eps0) * dy);
        // sigzmax = (m+1) / (150 * pi * sqrt((eps(i0,j,k)+eps(i0,j+1,k))/2/eps0) * dz);
        sigxmax = (m+1) / (150 * pi * sqrt(1) * dx);
        sigymax = (m+1) / (150 * pi * sqrt(1) * dy);
        sigzmax = (m+1) / (150 * pi * sqrt(1) * dz);
    }
    if(nn == 2)
    {
        // sigxmax = (m+1) / (150 * pi * sqrt((eps(i0,j,k)+eps(i0,j,k+1))/2/eps0) * dx);
        // sigymax = (m+1) / (150 * pi * sqrt((eps(i0,j,k)+eps(i0,j,k+1))/2/eps0) * dy);
        // sigzmax = (m+1) / (150 * pi * sqrt((eps(i0,j,k)+eps(i0,j,k+1))/2/eps0) * dz);
        sigxmax = (m+1) / (150 * pi * sqrt(1) * dx);
        sigymax = (m+1) / (150 * pi * sqrt(1) * dy);
        sigzmax = (m+1) / (150 * pi * sqrt(1) * dz);
    }
...
void PML_parameter_E(int nn, int i, int j, int k)
{
    int i0 = i - rangex[myRank] + order;
    if(nn == 0)
    {
        // sigxmax = (m+1) / (150 * pi * sqrt((eps(i0,j,k)+eps(i0,j+1,k+1)+eps(i0,j+1,k)+eps(i0,j,k+1))/4/eps0) * dx);
        // sigymax = (m+1) / (150 * pi * sqrt((eps(i0,j,k)+eps(i0,j+1,k+1)+eps(i0,j+1,k)+eps(i0,j,k+1))/4/eps0) * dy);
        // sigzmax = (m+1) / (150 * pi * sqrt((eps(i0,j,k)+eps(i0,j+1,k+1)+eps(i0,j+1,k)+eps(i0,j,k+1))/4/eps0) * dz);
        sigxmax = (m+1) / (150 * pi * sqrt(1) * dx);
        sigymax = (m+1) / (150 * pi * sqrt(1) * dy);
        sigzmax = (m+1) / (150 * pi * sqrt(1) * dz);


    // if(i >=8 && i <=12 && j == 50 && k >= 28 && k <= 32){
    //     cout << "PML_E: " << i << " " << j << " " << " " << k << \\
    //     " " << sigxmax << " " << sigymax << " " << sigzmax << endl;
    // }

    }
    if(nn == 1)
    {
        // sigxmax = (m+1) / (150 * pi * sqrt((eps(i0,j,k)+eps(i0+1,j,k+1)+eps(i0+1,j,k)+eps(i0,j,k+1))/4/eps0) * dx);
        // sigymax = (m+1) / (150 * pi * sqrt((eps(i0,j,k)+eps(i0+1,j,k+1)+eps(i0+1,j,k)+eps(i0,j,k+1))/4/eps0) * dy);
        // sigzmax = (m+1) / (150 * pi * sqrt((eps(i0,j,k)+eps(i0+1,j,k+1)+eps(i0+1,j,k)+eps(i0,j,k+1))/4/eps0) * dz);
        sigxmax = (m+1) / (150 * pi * sqrt(1) * dx);
        sigymax = (m+1) / (150 * pi * sqrt(1) * dy);
        sigzmax = (m+1) / (150 * pi * sqrt(1) * dz);
    // if(i>=9 && i<=15 && j == 50 && k>=28 && k<=32){
    //     FILE *ftest;
    //     ftest = fopen("test_1.txt","at");
    //     fprintf(ftest, "PML: %d %d %d %e %e %e %e %e\n", i, j, k, eps(i,j,k),(eps(i0,j,k)+eps(i0,j+1,k+1))/2 );
    //     fclose(ftest);
    ...
    if(nn == 2)
    {
        // sigxmax = (m+1) / (150 * pi * sqrt((eps(i0,j,k)+eps(i0+1,j+1,k)+eps(i0+1,j,k)+eps(i0,j+1,k))/4/eps0) * dx);
        // sigymax = (m+1) / (150 * pi * sqrt((eps(i0,j,k)+eps(i0+1,j+1,k)+eps(i0+1,j,k)+eps(i0,j+1,k))/4/eps0) * dy);
        // sigzmax = (m+1) / (150 * pi * sqrt((eps(i0,j,k)+eps(i0+1,j+1,k)+eps(i0+1,j,k)+eps(i0,j+1,k))/4/eps0) * dz);
        sigxmax = (m+1) / (150 * pi * sqrt(1) * dx);
        sigymax = (m+1) / (150 * pi * sqrt(1) * dy);
        sigzmax = (m+1) / (150 * pi * sqrt(1) * dz);
    }
...
void updateH_PML()
{
    ...
    for (ii=pml_start; ii<pml_end; ii++)
    {
        ...
        PML_parameter_H(2,i,j,k);

        // if( i<=15 && j == 50 && k>=28 && k<=32){
        // FILE *ftest;
        // ftest = fopen("test_1.txt","at");
        // fprintf(ftest, "PML: %d %d %d %e %e %e\n", i, j, k, sigx, sigy,sigz );
        // fclose(ftest);
        // }
...
void updateE_PML()
{
    ...
    for (ii=pml_start; ii<pml_end; ii++)
    {
        ...
        // sigxmax = (m+1) / (150 * pi * sqrt((eps(i0,j,k)+eps(i0,j+1,k+1))/2/eps0) * dx);
        // sigymax = (m+1) / (150 * pi * sqrt((eps(i0,j,k)+eps(i0,j+1,k+1))/2/eps0) * dy);
        // sigzmax = (m+1) / (150 * pi * sqrt((eps(i0,j,k)+eps(i0,j+1,k+1))/2/eps0) * dz);
        ...
        PML_parameter_E(0,i,j,k);
        


        sig_halfyz = (sig(i0,j,k) + sig(i0,j+1,k+1) + sig(i0,j+1,k) + sig(i0,j,k+1))/4;
        eps_halfyz = (eps(i0,j+1,k+1) + eps(i0,j,k) + eps(i0,j+1,k) + eps(i0,j,k+1))/4;
        CA = (1.0 - sig_halfyz * dt/(2.0*eps_halfyz))/(1.0 + sig_halfyz * dt/(2.0*eps_halfyz));
        CB = (dt/eps_halfyz)/(1.0 + sig_halfyz*dt / (2.0*eps_halfyz));
        ...
        PML_parameter_E(1,i,j,k);
        sig_halfxz = (sig(i0,j,k) + sig(i0+1,j,k+1) + sig(i0+1,j,k) + sig(i0,j,k+1))/4;
        eps_halfxz = (eps(i0,j,k) + eps(i0+1,j,k+1) + eps(i0+1,j,k) + eps(i0,j,k+1))/4;
        CA = (1.0 - sig_halfxz*dt/(2.0*eps_halfxz))/(1.0 + sig_halfxz * dt/(2.0*eps_halfxz));
        CB = (dt/eps_halfxz)/(1.0 + sig_halfxz*dt / (2.0*eps_halfxz));
        ...
        PML_parameter_E(2,i,j,k);
        sig_halfxy = (sig(i0,j,k) + sig(i0+1,j+1,k) + sig(i0+1,j,k) + sig(i0,j+1,k))/4;
        eps_halfxy = (eps(i0,j,k) + eps(i0+1,j+1,k) + eps(i0+1,j,k) + eps(i0,j+1,k))/4;
        CA = (1.0 - sig_halfxy*dt/(2.0*eps_halfxy))/(1.0 + sig_halfxy * dt/(2.0*eps_halfxy));
        CB = (dt/eps_halfxy)/(1.0 + sig_halfxy*dt / (2.0*eps_halfxy));
...
void MPI_X()
{
    ...
    if (myRank<NUM_OF_PROCESS-1)
    {
        MPI_Sendrecv(&Ex(step_x,0,0), sendcount, MPI_DOUBLE, myRank+1, 1, &Ex(step_x+order,0,0), recvcount, MPI_DOUBLE, myRank+1, 3, MPI_COMM_WORLD, &status);
        MPI_Sendrecv(&Hx(step_x,0,0), sendcount, MPI_DOUBLE, myRank+1, 2, &Hx(step_x+order,0,0), recvcount, MPI_DOUBLE, myRank+1, 4, MPI_COMM_WORLD, &status);
    }

    //Passing message to left & receiving message from left
    if (myRank>0)
    {
        MPI_Sendrecv(&Ex(order,0,0), sendcount, MPI_DOUBLE, myRank-1, 3, &Ex(0,0,0), recvcount, MPI_DOUBLE, myRank-1, 1, MPI_COMM_WORLD, &status);
        MPI_Sendrecv(&Hx(order,0,0), sendcount, MPI_DOUBLE, myRank-1, 4, &Hx(0,0,0), recvcount, MPI_DOUBLE, myRank-1, 2, MPI_COMM_WORLD, &status);
    }
}
