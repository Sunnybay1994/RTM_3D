...
char tag[10];
int ntag;
int NUM_OF_PROCESS = 4, NUM_OF_THREADS = 8; //x3850: 2(Threads)*8(cores)*4(processors)
...
int main(int argc, char *argv[])
{
    ...
    if(argc >= 2){
        cout << "argc:" << argc << " argv[0]:" << argv[0]<< " argv[1]:" << argv[1];// add by mbw at 20180627
        ntag = atoi(argv[1]);
        sprintf(tag, "_%04d", ntag);
    }
    else{
        ntag = 0;
        sprintf(tag, "_%04d", ntag);
...
*/
	...
	for (it=0; it<nt; ++it)
	{
		...
		if (it % output_step_t_of_wavefield == 0)
        {
            ...
            if (myRank == 0){
                output_wavefield(it/output_step_t_of_wavefield,E_total);
            }
...
void add_source(){
    ...
    for (ii=0; ii<(int)mysrc.size(); ++ii)
    {
        ...
        // src[mysrc[ii]].component(i0,j,k) = src_pulse(ii,it);
